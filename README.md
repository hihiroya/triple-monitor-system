...
`revuestarlight` の X 監視は RSSHub では取得漏れが出るツリー親ポストと RT を拾うため、`x-profile-monitor.yml` の `x_profile_poll` で定期実行します。その他の X/Twitter source は引き続き `x-twitter-monitor.yml` で RSSHub の `/twitter/user/...` route を使います。
...
X/Twitter source は RSSHub の routeParams を使えます。スレッド付き投稿が通常 timeline に出ない場合は、対象 source の `rssUrl` に `/forceWebApi=1&count=50`、`/includeReplies=1&forceWebApi=1&count=50`、旧 routeParams 系の `/excludeReplies=0&forceWebApi=1&count=50` を段階的に試してください。これらで期待したポストが取れず、返信混入や空 feed のリスクだけが残る場合は、通常の `/twitter/user/account` に戻して安定運用を優先します。`/twitter/keyword/from:account` は Twitter API 404 で失敗するケースがあるため、現状は常用しません。Actions ログの `list snapshot` には取得 item 数、既読交差数、先頭 10 件の item ID サンプルが出ます。
...
現在は `revuestarlight-x-profile` を `group=x-profile` として登録し、`x-profile-monitor.yml` が 30 分間隔で定期実行します。RSSHub 側の `revuestarlight` source は重複通知を避けるため無効化しています。`maxAgeHours` で古すぎる投稿を除外し、過去ポストの大量通知を避けます。X GraphQL の query id が変わった場合は、`X_GQL_USER_BY_SCREEN_NAME`、`X_GQL_USER_TWEETS`、`X_GQL_USER_TWEETS_AND_REPLIES` 環境変数で一時的に差し替えられます。
