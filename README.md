...
X/Twitter source は RSSHub の routeParams を使えます。スレッド付き投稿が通常 timeline に出ない場合は、まず対象 source の `rssUrl` に `/forceWebApi=1&count=50` のような routeParams を追加して、RSSHub 側の取得結果に対象ポストが含まれるか確認してください。返信ポストまで必要な場合は、RSSHub image を最新 digest に更新したうえで `/includeReplies=1&forceWebApi=1&count=50`、または旧 routeParams 系の `/excludeReplies=0&forceWebApi=1&count=50` を段階的に試してください。user route でツリー投稿が出ない場合は、RSSHub 側で Latest 検索を使う `/twitter/keyword/from%3Aaccount` への切り替えも検証します。Actions ログの `list snapshot` には取得 item 数、既読交差数、先頭 10 件の item ID サンプルが出ます。
